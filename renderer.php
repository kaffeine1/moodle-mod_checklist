...
class mod_checklist_renderer extends plugin_renderer_base {
     ...
     * Card view is used when student is viewing their own checklist (not teacher view).
     *
     * @param output_status $status
     * @return bool
     */
    private function should_use_card_view(output_status $status): bool {
        return $status->is_updateform()
            && !$status->is_viewother()
            && $status->is_showcheckbox()
            && !$status->is_addown();
    }

    /**
     * Determine the item type based on display text for card styling.
     * Returns 'visto' for "Visto Fare" items, 'fatto' for "Fatto" items.
     *
     * @param string $displaytext
     * @return string
     */
    private function get_item_type(string $displaytext): string {
        $text = strtolower($displaytext);
        if (strpos($text, 'visto') !== false) {
            return 'visto';
        }
        if (strpos($text, 'fatto') !== false) {
            return 'fatto';
        }
        return 'default';
    }
    ...
    /**
     * Get icon for item type in card view.
     *
     * @param string $itemtype
     * @return string
     */
    private function get_item_icon(string $itemtype): string {
        switch ($itemtype) {
            case 'visto':
                return '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';
            case 'fatto':
            ...
            default:
                return '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>';
        }
    }

    /**
     * Get description for item type in card view.
     *
     * @param string $itemtype
     * @return string
     */
    private function get_item_description(string $itemtype): string {
        switch ($itemtype) {
            case 'visto':
                return 'Ho osservato questa procedura';
            case 'fatto':
                return 'Ho eseguito questa procedura';
            default:
                return '';
        }
    }
    ...
    public function checklist_items(
        ...
        $out = $this->output->box_start(
            'generalbox boxwidthwide boxaligncenter checklistbox',
            null,
            ['data-cmid' => $cmid]
        );
        ...
        if ($status->is_showprogressbar() && $progress && !$usecardview) {
            $out .= $this->progress_bars(
                ...
                $progress->allcompleteitems,
                $progress->requiredcompleteitems
            );
        }
        ...
        if (!$items) {
            ...
            if ($usecardview) {
                ...
                foreach ($items as $checkitem) {
                    if (!$checkitem->hidden && !$checkitem->is_heading() && !$checkitem->is_checked_student()) {
                        $allchecked = false;
                        break;
                    }
                }
                $cardclass = $allchecked ? 'checklist-card all-completed' : 'checklist-card';
                $out .= '<div class="' . $cardclass . '" id="checklistouter">';
                $out .= '<div class="checklist-card-body">';
            } else {
            ...
            foreach ($items as $item) {
                ...
                // Card view rendering.
                if ($usecardview && !$item->is_heading()) {
                    $itemtype = $this->get_item_type($item->displaytext);
                    $rowclass = 'checklist-item-row item-' . $itemtype;
                    if ($ischecked) {
                        $rowclass .= ' is-checked';
                    }
                    ...
                    // Icon.
                    $out .= '<div class="checklist-item-icon">';
                    $out .= $this->get_item_icon($itemtype);
                    $out .= '</div>';

                    // Content.
                    $out .= '<div class="checklist-item-content">';
                    $out .= '<div class="checklist-item-label">';
                    $out .= '<span class="check-icon">&#10003;</span>';
                    $out .= format_string($item->displaytext);
                    $out .= '</div>';
                    $description = $this->get_item_description($itemtype);
                    if ($description) {
                        $out .= '<div class="checklist-item-description">' . $description . '</div>';
                    }
                    $out .= '</div>';

                    // Toggle switch.
                    $out .= '<div class="checklist-toggle-wrapper">';
                    $out .= '<input type="checkbox" class="checklist-toggle checklistitem' . $checkclass . '"';
                    $out .= ' data-cmid="' . $cmid . '" name="items[]" id="item' . $item->id . '"';
                    $out .= $checked . ' value="' . $item->id . '" />';
                    $out .= '</div>';

                    $out .= '</div>'; // End checklist-item-row.
                } else {
                    // Original rendering for non-card view or headings.
                    $out .= '<li>';
                    ...
                    $out .= '</li>';
                } // End of non-card view item rendering.

                $inline = self::form_inline_class();

                // Output any user-added items (not in card view).
                if (!$usecardview && $useritems) {
            ...
            if ($usecardview) {
                $out .= '</div></div>'; // Close checklist-card-body and checklist-card.
            } else {
                $out .= '</ol>';
            }
            ...
            switch ($item->get_link_type()) {
                case checklist_item::LINK_MODULE:
                    $icon = $this->output->pix_icon(
                        'follow_link',
                        get_string('linktomodule', 'checklist'),
                        'mod_checklist'
                    );
                    break;
                ...
                case checklist_item::LINK_URL:
                    $icon = $this->output->pix_icon(
                        'follow_link',
                        get_string('linktourl', 'checklist'),
                        'mod_checklist'
                    );
    ...
    protected function edit_link_form(output_status $status, $item = null) {
        ...
        if ($status->is_allowcourselinks()) {
            ...
            $out .= html_writer::select(
                checklist_class::get_linkable_courses(),
                'linkcourseid',
                $selected,
